...
fn write_clipboard_text(text: String) -> Result<(), String> {
    ...
    if status.success() {
        let output = std::process::Command::new("/usr/bin/pbpaste")
            .output()
            .map_err(|error| format!("Unable to verify clipboard text: {error}"))?;
        let written_text = String::from_utf8_lossy(&output.stdout);
        if written_text == text {
            Ok(())
        } else {
            Err("Clipboard write verification failed.".to_string())
        }
    } else {
