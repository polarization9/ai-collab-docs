# AI Markdown Reviewer 产品外壳与 Codex 连接 PRD
...
AI Markdown Reviewer 当前已经具备本地 Markdown 阅读、Mermaid 增强、批注、MCP 协作和轻量编辑能力。下一步需要回答两个更基础的问题：

1. 用户如何自然地把一份 `.md` 文档交给这个产品打开，而不是记命令、找端口、复制路径。
2. 当文档来自 Codex 讨论时，产品如何把批注和修改任务送回正确的 Codex 上下文里。

这个模块不只是“把 Web App 包成桌面 App”。它的核心目标是让 AI Markdown Reviewer 从一个 localhost 工具升级为一个本地文档协作入口，并和 Codex 建立可恢复、可路由、可自动投递的连接。

## 3. 产品定位

AI Markdown Reviewer 的产品外壳优先采用本地桌面 App 形态。桌面 App 负责文件打开、系统集成、后台服务和本地权限；现有 Web UI 继续作为主要阅读和编辑界面。
...
> 用户双击一份 Markdown 文档，或在 Codex 产出文档后选择“用 AI Markdown Reviewer 打开”。App 打开文档并识别它是否来自某个 Codex 会话。用户在文档中留下批注后，系统可以把新批注事件交给正确的 Codex 会话处理。Codex 在该会话已有上下文中读取批注、修改正文、回复批注并更新状态。
...
- 用户可以在系统中选择 AI Markdown Reviewer 打开 Markdown 文件。
- 支持双击文件、右键打开方式、拖入窗口打开。
- 后续可引导用户将 AI Markdown Reviewer 设置为默认 Markdown 打开方式。
...
4. 目标 Codex 会话按指令调用 AI Markdown Reviewer MCP。
5. MCP 写入接续对话连接信息。
6. App 刷新后展示“已配置接续对话”。
...
开启自动监控不等于产品自己理解上下文，也不等于产品自动修改正文。AI Markdown Reviewer 只负责把批注事件送到正确会话；Codex 仍通过 MCP 读取文档和批注，并在目标会话上下文中决定回复、修改正文或更新状态。
...
- Codex 收到任务后，第一步应调用 AI Markdown Reviewer MCP 读取批注上下文，而不是依赖 prompt 中的摘要。
...
```text
AI Markdown Reviewer 有一条新的批注任务需要处理。
...
1. 调用 AI Markdown Reviewer MCP 读取这条批注：
...
4. 用户选择用 AI Markdown Reviewer 打开。
