...
后续可以在文档状态区补充未保存状态和外部更新状态；P0 不把这些状态强行塞进 tab 本身，避免顶部文档条过重。
...
- 支持从启动页恢复上次关闭前的文档。
- 支持清理最近文件记录。
- 不提供独立“打开示例文档”入口，避免空态变成教学或营销页面；首次体验所需的 Quickstart 示例可以作为最近文件中的一条默认记录出现。
...
- 空态启动页中的独立“打开示例文档”入口；内置 Quickstart 示例只通过最近文件出现。
- 语义级向量匹配。
- 大段重写后的自动语义恢复。
- 多人同时编辑下的复杂冲突合并。
- 无授权读取全部 Codex 历史会话。
...
2. 系统根据系统语言选择默认界面语言。
3. 系统加载默认配色方案。
4. 如果是首次初始化，系统把内置 `Margent Quickstart.md` 复制到用户可写目录，并写入最近文件。
5. 系统检测是否有待打开文档。
6. 如果没有文档，展示空态启动页和最近文件。
7. 用户可以点击最近文件中的 Quickstart 示例，也可以点击打开 Markdown 文件。
8. 系统打开并加载用户选择的 `.md` 文件。
...
- 最近打开文件作为辅助入口放在主操作下方，文件不存在或打开失败时只显示轻量状态。
...
- 空态启动页不需要独立“打开示例文档”入口；Quickstart 示例通过最近文件预置。
