...
1. 打开 Margent。
2. 空态启动页下方的最近文件里出现 `Margent Quickstart.md`。
3. 点击后进入真实阅读界面。
4. 批注写入 `~/Documents/Margent/Margent Quickstart.review.json`。
5. 如果连接 Codex，连接信息写入 `~/Documents/Margent/Margent Quickstart.codex.json`。

## 4. 不提供恢复入口

如果用户删除 `~/Documents/Margent/Margent Quickstart.md`，最近文件中可以显示“文件不存在”。Margent 不需要提供“重新创建示例文档”的按钮，也不在下次启动时重新复制。
...
- 已新增内置示例文档模板。
- 已支持首次复制到 `~/Documents/Margent/Margent Quickstart.md`。
- 已支持首次把示例文档写入最近文件。
...
- 已新增英文 Quickstart 文档，当前先作为独立示例文件提供；首次最近文件仍默认注入中文 `Margent Quickstart.md`。
